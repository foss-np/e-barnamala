...
class activitiesMainPanel(wx.Panel):
    def __init__(self, parent, id, mainPanel, currentLesson):
        ...
        if buttonImages[10] != '':
            memoryPairsImg = wx.Bitmap(join(config.buttonsPath, buttonImages[10]), wx.BITMAP_TYPE_PNG)
            self.memoryPairsButton = wx.BitmapButton(self.mainPanel, 11, memoryPairsImg, (300,300), style = wx.NO_BORDER)
            self.memoryPairsButton.SetBackgroundColour(config.backgroundColour)
            self.memoryPairsButton.Bind(wx.EVT_BUTTON, self.OnMemoryPairsGame, id=11)
        else:
            pass
