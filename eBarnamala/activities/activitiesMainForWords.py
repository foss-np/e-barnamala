...
class activitiesMainPanel(wx.Panel):
    def __init__(self, parent, id, mainPanel, currentLesson):
        ...
        if self.currentLesson == "colour":
            ...
            buttonImages.append('memoryPairs.png')
        ...
        if buttonImages[4] != '':
            memoryPairsImg = wx.Bitmap(join(config.buttonsPath, buttonImages[4]), wx.BITMAP_TYPE_PNG)
            self.memoryPairsButton = wx.BitmapButton(self.mainPanel, wx.ID_ANY, memoryPairsImg, (300,200), style = wx.NO_BORDER)
            self.memoryPairsButton.SetBackgroundColour(config.backgroundColour)
            self.memoryPairsButton.Bind(wx.EVT_BUTTON, self.OnMemoryPairsGame, id=self.memoryPairsButton.GetId())
        else:
            pass
    ...
    def OnMemoryPairsGame(self, event):
        self.mainPanel.Show(False)
        self.mainWin.Show(False)
        memoryPairs.memoryPairs(self.parent, -1, self.mainWin, self.mainPanel, self.currentLesson)
