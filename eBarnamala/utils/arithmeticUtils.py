...
def nextNumberBigSmall():
    numberSequence = random.sample(range(10), 3)
    return numberSequence[0], numberSequence[1], numberSequence[2] 

def previousNumberBigSmall():
    numberSequence[0], numberSequence[1], numberSequence[2] = nextNumberBigSmall()
    return numberSequence[0], numberSequence[1], numberSequence[2]

def findBigSmallPosition(num1, num2, num3):
    tempList = []
    tempList.append(num1)
    tempList.append(num2)
    tempList.append(num3)
    maxNumberPosition = tempList.index(max(tempList))
    minNumberPosition = tempList.index(min(tempList))
    return maxNumberPosition, minNumberPosition
