...
class learnArithmeticPanel(wx.Panel):
    def __init__(self, parent, id, mainPanel):
        ...
        self.oneMoreButton = wx.BitmapButton(self.menuPanel, wx.ID_ANY, oneMoreImg, (5,10), style = wx.NO_BORDER)
        ...
        self.oneLessButton = wx.BitmapButton(self.menuPanel, wx.ID_ANY, oneLessImg, (135,10), style = wx.NO_BORDER)
        ...
        moreLessImg = wx.Bitmap(join(config.buttonsPath,'moreLess.png'), wx.BITMAP_TYPE_PNG)
        self.moreLessButton = wx.BitmapButton(self.menuPanel, wx.ID_ANY, moreLessImg, (265,10), style = wx.NO_BORDER)
        self.moreLessButton.SetBackgroundColour(config.backgroundColour)
        self.moreLessButton.Bind(wx.EVT_BUTTON, self.OnMoreLessButton, id=self.moreLessButton.GetId())

        bigSmallImg = wx.Bitmap(join(config.buttonsPath,'bigSmall.png'), wx.BITMAP_TYPE_PNG)
        self.bigSmallButton = wx.BitmapButton(self.menuPanel, wx.ID_ANY, bigSmallImg, (400,10), style = wx.NO_BORDER)
        self.bigSmallButton.SetBackgroundColour(config.backgroundColour)
        self.bigSmallButton.Bind(wx.EVT_BUTTON, self.OnBigSmallButton, id=self.bigSmallButton.GetId())

        simplePlusImg = wx.Bitmap(join(config.buttonsPath,'simplePlus.png'), wx.BITMAP_TYPE_PNG)
        self.simplePlusButton = wx.BitmapButton(self.menuPanel, wx.ID_ANY, simplePlusImg, (535,10), style = wx.NO_BORDER)
        ...
        self.simpleMinusButton = wx.BitmapButton(self.menuPanel, wx.ID_ANY, simpleMinusImg, (665,10), style = wx.NO_BORDER)
    ...
    def OnBigSmallButton(self, event):
        self.currentLesson = "bigSmall"
        self.arithmeticSignLabel.Show(False)
        self.equalToLabel.Show(False)
        self.displayLabel3.Show(True)
        self.displayLabel5.Show(True)
        self.displayLabel7.Show(True)
        self.displayLabel4.Show(False)
        self.displayLabel6.Show(False)
        self.displayLabel9.Show(False)
        self.displayLabel11.Show(False)
        self.currentNumber, self.numberArithmetic, self.nextNumber = arithmeticUtils.nextNumberBigSmall()
        bigPosition, smallPosition = arithmeticUtils.findBigSmallPosition(self.currentNumber, self.numberArithmetic, self.nextNumber)
    ...
    def OnMoreLessButton(self, event):
        self.currentLesson = "moreLess"
        self.arithmeticSignLabel.Show(False)
        self.equalToLabel.Show(False)
        self.displayLabel3.Show(False)
        self.displayLabel5.Show(False)
        self.displayLabel7.Show(False)
        self.displayLabel4.Show(False)
        self.displayLabel6.Show(False)
        self.displayLabel9.Show(False)
        self.displayLabel11.Show(False)
    ...
    def OnPreviousArrow(self, event):
        ...
        if (self.currentLesson == "simplePlus") or (self.currentLesson == "simpleMinus"):
            ...
            self.displayLabel10.SetLabel(loadDataArithmetic.numberNames[int(self.numberArithmetic)])
            numberArithmeticImg = arithmeticUtils.nextImage(self.currentLesson, self.numberArithmetic)
        if (self.currentLesson == "bigSmall") or (self.currentLesson == "moreLess"):
            self.currentNumber, self.numberArithmetic, self.nextNumber = arithmeticUtils.nextNumberBigSmall()
            bigPosition, smallPosition = arithmeticUtils.findBigSmallPosition(self.currentNumber, self.numberArithmetic, self.nextNumber)
            self.displayBigSmall(bigPosition, smallPosition)
            self.displayLabel5.SetLabel(self.mreplace(str(self.numberArithmetic),"engToNep"))
    ...
    def displayBigSmall(self, bigPosition, smallPosition):
        messageBig = ''
        messageSmall = ''
        if self.currentLesson == "bigSmall":
            messageBig = messages.big
            messageSmall = messages.small
        if self.currentLesson == "moreLess":
            messageBig = messages.more
            messageSmall = messages.less
        self.displayLabel0.Show(False)
        self.displayLabel1.Show(False)
        self.displayLabel2.Show(False)
        if bigPosition == 0:
            self.displayLabel0.Show(True)
            self.displayLabel0.SetLabel(messageBig)
        if bigPosition == 1:
            self.displayLabel1.Show(True)
            self.displayLabel1.SetLabel(messageBig)
        if bigPosition == 2:
            self.displayLabel2.Show(True)
            self.displayLabel2.SetLabel(messageBig)
        if smallPosition == 0:
            self.displayLabel0.Show(True)
            self.displayLabel0.SetLabel(messageSmall)
        if smallPosition == 1:
            self.displayLabel1.Show(True)
            self.displayLabel1.SetLabel(messageSmall)
        if smallPosition == 2:
            self.displayLabel2.Show(True)
            self.displayLabel2.SetLabel(messageSmall)
